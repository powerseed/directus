		...
		if (schema.default_value !== undefined && schema.default_value !== existingColumn.default_value) {
			if (schema.default_value === null) {
				...
				if (isNowFunction || isCurrentTimestamp) {
					...
					await this.knex.raw(
						`ALTER TABLE ?? ALTER COLUMN ?? SET DEFAULT CURRENT_TIMESTAMP(${Number(precision)})`,
					...
					await this.knex.raw(`ALTER TABLE ?? ALTER COLUMN ?? SET DEFAULT ${defaultValue}`, [
						collection,
						field.field,
					]);
				} else {
					...
					await this.knex.raw(
						`ALTER TABLE ?? ALTER COLUMN ?? SET DEFAULT ${this.toSqlLiteral(defaultValue)}`,
						[collection, field.field],
					);
				}
			}
